        ...
        try:
            ...
            if total_encontrados == 0:
                ...
                if records:
                    ...
                    # 3. Gráfico: Hora promedio de llegada por semana
                    st.markdown("### 3. Hora promedio de llegada por semana")
                    
                    df["Segundos_Llegada"] = horas_dt.dt.hour * 3600 + horas_dt.dt.minute * 60 + horas_dt.dt.second
                    df_prom_semana = df.groupby("Semana", as_index=False)["Segundos_Llegada"].mean()
                    df_prom_semana["Fecha_Semana"] = pd.to_datetime(df_prom_semana["Semana"].str.replace("Semana del ", ""))
                    df_prom_semana = df_prom_semana.sort_values("Fecha_Semana")

                    df_prom_semana["Hora_Promedio_dt"] = pd.to_datetime(df_prom_semana["Segundos_Llegada"], unit="s")
                    df_prom_semana["Hora_Texto"] = df_prom_semana["Hora_Promedio_dt"].dt.strftime("%H:%M:%S")

                    fig_prom_semana = px.line(
                        df_prom_semana,
                        x="Semana",
                        y="Hora_Promedio_dt",
                        text="Hora_Texto",
                        title="Evolución de la Hora Promedio de Llegada por Semana",
                        markers=True
                    )
                    fig_prom_semana.update_traces(textposition="top center")
                    fig_prom_semana.update_yaxes(title_text="Hora Promedio", tickformat="%H:%M")
                    fig_prom_semana.update_xaxes(type="category", tickmode="linear")

                    st.plotly_chart(fig_prom_semana, use_container_width=True)
