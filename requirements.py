    ...
    if not raw_email or not raw_pass:
        ...
        try:
            ...
            if total_encontrados == 0:
                ...
                email_ids_procesar = list(reversed(email_ids))[:100]
                ...
                if records:
                    ...
                    # --- TABLA REGISTRO DETALLADO CON FORMATO CONDICIONAL ---
                    st.subheader("📋 Registro Detallado")

                    def resaltar_estado(val):
                        if val == "Después de las 9:35":
                            return "background-color: #ffc7ce; color: #9c0006; font-weight: bold;"
                        elif val == "Antes de las 9:35":
                    ...
                    # --- MÉTRICAS Y GRÁFICOS CON PROMEDIOS ---
                    st.subheader("📊 Análisis Gráfico con Promedios")

                    # 1. Gráfico: Días con más faltas / atrasos
                    st.markdown("### 1. Días con mayor cantidad de atrasos")
                    dias_orden = ["Lunes", "Martes", "Miércoles", "Jueves", "Viernes", "Sábado", "Domingo"]
                    df_dias = df["Día de la semana"].value_counts().reindex(dias_orden).fillna(0).reset_index()
                    df_dias.columns = ["Día de la semana", "Cantidad"]

                    # Cálculo del promedio de atrasos por día hábil (Lunes a Viernes)
                    dias_habiles = df_dias[df_dias["Día de la semana"].isin(["Lunes", "Martes", "Miércoles", "Jueves", "Viernes"])]
                    promedio_por_dia = dias_habiles["Cantidad"].mean()

                    st.metric(
                        label="Promedio de Atrasos por Día de la Semana (Lunes a Viernes)",
                        value=f"{promedio_por_dia:.1f} atrasos"
                    )
                    ...
                    fig_dias = px.bar(
                        ...
                        title="Frecuencia de Atrasos por Día"
                    )
                    ...
                    with col_graf1:
                        # 2. Gráfico: Cuántos días vas a la semana
                        st.markdown("### 2. Días asistidos/registrados por semana")
                        df_semana_dias = df.groupby("Semana")["Fecha"].nunique().reset_index()
                        df_semana_dias.columns = ["Semana", "Días con Atraso"]
                        
                        promedio_dias_semana = df_semana_dias["Días con Atraso"].mean()

                        st.metric(
                            label="Promedio de Días con Atraso por Semana",
                        ...
                        fig_semana = px.line(
                            ...
                            y="Días con Atraso",
                            markers=True,
                            title="Días a la semana con registro de atraso",
                    ...
                    with col_graf2:
                        # 3. Gráfico: A qué hora llegas
                        st.markdown("### 3. Distribución de hora de llegada")
                        
                        # Cálculo de hora promedio
